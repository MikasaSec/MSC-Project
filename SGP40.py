...
class SGP40:
    ...
    def raw(self):
        ...
        time.sleep(0.25)
        Rbuf = self.Read()
        return ((int(Rbuf[0]) << 8) | Rbuf[1])#将读取的数据块中的两个字节（Rbuf[0]和Rbuf[1]）合并成一个整数值
    #获取raw gas value
    ...
    def measureRaw(self, temperature, humidity):
        ...
        WITH_HUM_COMP[7] = int(crct) #将补偿参数和校验值填充到命令数组WITH_HUM_COMP中的相应位置
        self.write_block(WITH_HUM_COMP) #将命令发送给设备，触发测量过程
        time.sleep(0.5)
        Rbuf = self.Read()
        return ((int(Rbuf[0]) << 8) | Rbuf[1]) #将读取的数据块中的两个字节（Rbuf[0]和Rbuf[1]）合并成一个整数值
    #用于测量经过温湿度补偿的气体数值
...
if __name__ == '__main__':
    ...
    try:
        while True:
            print("gas: ", sgp.measureRaw(25, 50))
            time.sleep(1)
